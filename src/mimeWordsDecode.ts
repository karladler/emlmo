...
export function mimeWordsDecode(input: string): string {
  ...
  // Remove whitespace only between encoded words, not within decoded content
...
function decodeBytes(bytes: Uint8Array, charset: string): string {
  ...
  // For Windows-1252, always use manual decoder to handle special characters
  if (charset === 'windows-1252') {
    return decodeWin1252(bytes)
  }

  // Prefer TextDecoder if available
  ...
  // Manual fallback (Windows-1252 + Latin-1)
  return decodeWin1252(bytes)
}
