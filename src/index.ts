...
import { base64Decode, base64Encode, base64ToUint8Array } from './base64';
    ...
    if (data.attachments) {
      for (let i = 0; i < data.attachments.length; i++) {
        ...
        if (typeof attachment.data === 'string') {
          const content = base64Encode(attachment.data);
...
function read(
  ...
  function _append(headers: EmlHeaders, content: string | Uint8Array | Attachment, result: ReadedEmlJson) {
    ...
    if (!contentDisposition && contentType && contentType.indexOf('text/html') >= 0) {
      ...
      if (result_name) {
        attachment.name = mimeWordsDecode(decodeURI(result_name));
      }
